...
describe('deriveMorningSteps — steps grow from definitions, never hardcoded', () => {
  it('the standard seed yields 3 steps: sleep pair + screen_time + mood (spec §9.1)', () => {
  ...
  it('inserting a new metric grows the form by a step (final M1 checklist item)', () => {
...
describe('deriveEveningSteps — scales grouped on one screen per visibleWhen (spec §9.3)', () => {
  ...
  it('workday: 3 scales on one screen + chips + note = 3 steps', () => {
  ...
  it('day off: 5 scales on one screen (server already filtered visibleWhen)', () => {
